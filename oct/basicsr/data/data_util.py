...
def read_img_seq(path, require_mod_crop=False, scale=1):
    ...
    imgs = [cv2.imread(v,0).astype(np.float32) / 255. for v in img_paths]
    if require_mod_crop:
...
def paired_paths_from_folder_two_input(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    Args:
        folders (list[str]): A list of folder path. The order of list should
            be [input_folder, gt_folder].
...
def paired_paths_from_folder_three_input(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    Args:
        folders (list[str]): A list of folder path. The order of list should
            be [input_folder, gt_folder].
...
def paired_paths_from_folder_flow(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    assert len(keys) == 3, (
        'The len of keys should be 2 with [input_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder ,shift_folder = folders
    input_key, gt_key, shift_key = keys

    input_paths = list(scandir(input_folder))
    gt_paths = list(scandir(gt_folder))
    shift_paths = list(scandir(shift_folder))
    ...
    for idx in range(len(gt_paths)):
        gt_path = gt_paths[idx]
        basename, ext = osp.splitext(osp.basename(gt_path))
        shift_path = shift_paths[idx]
        basename_shift, ext_shift = osp.splitext(osp.basename(shift_path))
        input_path = input_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(input_path))
        input_name = f'{filename_tmpl.format(basename)}{ext_input}'
        input_path = osp.join(input_folder, input_name)
        ...
        shift_name = f'{filename_tmpl.format(basename)}{ext_shift}'
        shift_path = osp.join(shift_folder, shift_name)
        gt_path = osp.join(gt_folder, gt_path)
