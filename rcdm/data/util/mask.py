...
def boundarymask(img_shape, dtype='uint8'):
    """Generate mask in ndarray from bbox.
    ...
    res = random.sample(range(10, width-10), N)

    for i in range(len(res)-1):
        rand_move = random.randint(-1,1) # -1,0,1
        rand_up = random.randint(0,1) # -1,0,1
        if rand_move >= 0:
            dy1 = random.randint(1,15)
            # dx = random.randint(1,4)
            if rand_up > 0:
                mask[0:dy1,res[i]: res[i+1], :] = 1
            else:
                mask[-dy1:,res[i]: res[i+1], :] = 1

    # dy1 = random.randint(0,15)
    # mask[0:dy1,:, :] = 1

    # dy2 = random.randint(0,15)
    # mask[-dy2:,:, :] = 1
    rand_x = random.randint(-2,3) # -2,-1,0,1,2,3
    if rand_x>0:
...
def strip2mask(img_shape,  dtype='uint8'):
    """Generate mask in ndarray from bbox.
